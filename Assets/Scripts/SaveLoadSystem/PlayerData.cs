...
namespace SaveLoadSystem
{
    ...
    public class PlayerData
    {
        ...
        public PlayerData(Vector3 playerPosition, InventorySystem.Inventory playerInventory, int gun, int skinIndex, int character, int health, int ammoCount, int maxHealth)
        {
            this.health = health;
            ...
            for (int i = 0; i < playerInventory.slots.Length; ++i)
            {
                ...
                else if (playerInventory.buttonSlots[i].transform.GetChild(0).GetComponent<InventorySystem.GunSelect>() != null)
                {
                    var dmg = playerInventory.buttonSlots[i].transform.GetChild(0).GetComponent<InventorySystem.GunSelect>().damage;
                    if (dmg == 5)
                        inventory[i] = 3;
                    else
                        inventory[i] = 4;
                ...
                {
                    var otft = playerInventory.buttonSlots[i].transform.GetChild(0).GetComponent<InventorySystem.OutfitSelect>().skinIndex;
                    if (otft == 1)
                        inventory[i] = 6;
                    else
                        inventory[i] = 7;
