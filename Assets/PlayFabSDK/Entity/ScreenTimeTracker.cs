...
namespace PlayFab.Public {
  ...
  public class ScreenTimeTracker : IScreenTimeTracker {
    ...
    public void ClientSessionStart(string entityId, string entityType, string playFabUserId) {
      ...
      var eventInfo = new EventsModels.EventContents();
    ...
    public void OnApplicationFocus(bool isFocused) {
      ...
      var focusStateDuration = 0.0;

      if (initialFocus) focusId = Guid.NewGuid();
      ...
      if (isFocused) {
        ...
        if (!initialFocus) {
          ...
          if (focusStateDuration < 0) focusStateDuration = 0;
        }
      }
      else {
