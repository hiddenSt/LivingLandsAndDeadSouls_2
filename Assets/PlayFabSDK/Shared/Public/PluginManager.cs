...
namespace PlayFab {
  public class PluginManager {
    private Dictionary<PluginContractKey, IPlayFabPlugin> plugins =
      new Dictionary<PluginContractKey, IPlayFabPlugin>(new PluginContractKeyComparator());
      ...
      if (!plugins.TryGetValue(key, out plugin)) {
        ...
        switch (contract) {
          case PluginContract.PlayFab_Serializer:
            plugin = CreatePlugin<Json.SimpleJsonInstance>();
            break;
          case PluginContract.PlayFab_Transport:
            plugin = CreatePlayFabTransportPlugin();
            break;
          default:
    ...
    private void SetPluginInternal(IPlayFabPlugin plugin, PluginContract contract, string instanceName) {
      if (plugin == null) throw new ArgumentNullException("plugin", "Plugin instance cannot be null");
