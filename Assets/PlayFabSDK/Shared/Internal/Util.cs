...
namespace PlayFab.Internal {
  public static class PlayFabUtil {
    ...
    public static string timeStamp => DateTime.Now.ToString(_defaultDateTimeFormats[DEFAULT_LOCAL_OUTPUT_INDEX]);

    public static string utcTimeStamp => DateTime.UtcNow.ToString(_defaultDateTimeFormats[DEFAULT_UTC_OUTPUT_INDEX]);
    ...
    public static string ReadAllFileText(string filename) {
      ...
      using (var fs = new FileStream(filename, FileMode.Open)) {
        using (var br = new BinaryReader(fs)) {
          while (br.BaseStream.Position != br.BaseStream.Length) _sb.Append(br.ReadChar());
        }
      }
    ...
    internal static string GetLocalSettingsFileProperty(string propertyKey) {
      ...
      var currDirEnvFile = Path.Combine(currDir, _localSettingsFileName);
      ...
      if (File.Exists(currDirEnvFile)) {
        ...
        var tempDirEnvFile = Path.Combine(tempDir, _localSettingsFileName);

        if (File.Exists(tempDirEnvFile)) envFileContent = ReadAllFileText(tempDirEnvFile);
      }
      ...
      if (!string.IsNullOrEmpty(envFileContent)) {
        ...
        try {
          object result;
          if (envJson.TryGetValue(propertyKey, out result)) return result == null ? null : result.ToString();
