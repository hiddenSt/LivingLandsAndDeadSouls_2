 ...
 DynamicObject,
  ...
  public
    ...
    public static string SerializeObject(object json, IJsonSerializerStrategy jsonSerializerStrategy = null) {
      ...
      var success = SerializeValue(jsonSerializerStrategy, json, _serializeObjectBuilder);
        ...
        if (c == '"') {
          ...
          if (c == '"') {
            ...
            if (remainingLength >= 4) {
              ...
              uint codePoint;
              if (!(success = uint.TryParse(json.Substring(index, 4), NumberStyles.HexNumber,
              ...
              if (0xD800 <= codePoint && codePoint <= 0xDBFF) // if high surrogate
              {
                ...
                if (remainingLength >= 6) {
                  uint lowCodePoint;
                  if (json.Substring(index, 2) == "\\u" && uint.TryParse(json.Substring(index + 2, 4),
                    NumberStyles.HexNumber, CultureInfo.InvariantCulture, out lowCodePoint))
    ...
    private static object ParseNumber(string json, ref int index, ref bool success) {
      ...
      object returnNumber;
      var str = json.Substring(index, charLength);
      if (str.IndexOf(".", StringComparison.OrdinalIgnoreCase) != -1 ||
          str.IndexOf("e", StringComparison.OrdinalIgnoreCase) != -1) {
        double number;
        success = double.TryParse(json.Substring(index, charLength), NumberStyles.Any, CultureInfo.InvariantCulture,
          out number);
        returnNumber = number;
      }
      else if (str.IndexOf("-", StringComparison.OrdinalIgnoreCase) == -1) {
        ulong number;
        success = ulong.TryParse(json.Substring(index, charLength), NumberStyles.Any, CultureInfo.InvariantCulture,
          out number);
        returnNumber = number;
      }
      else {
        long number;
        success = long.TryParse(json.Substring(index, charLength), NumberStyles.Any, CultureInfo.InvariantCulture,
          out number);
        returnNumber = number;
      }
    ...
    private static bool SerializeValue(IJsonSerializerStrategy jsonSerializerStrategy, object value,
      ...
      if (value == null) {
        ...
        var genArgs = ReflectionUtils.GetGenericTypeArguments(type);
 ...
 new DataContractJsonSerializerStrategy());
  ...
  public
    ...
    public virtual object DeserializeObject(object value, Type type) {
      ...
      var valueIsUlong = value is ulong;
      ...
      var isNumberType = PlayFabSimpleJson.NumberTypes.IndexOf(type) != -1;
      var isEnumType = type.GetTypeInfo().IsEnum;
    ...
    protected virtual bool TrySerializeUnknownTypes(object input, out object output) {
      ...
      foreach (var getter in getters) {
        ...
        if (obj.ContainsKey(jsonKey))
          throw new Exception("The given key is defined multiple times in the same type: " + input.GetType().Name +
                              "." + jsonKey);
