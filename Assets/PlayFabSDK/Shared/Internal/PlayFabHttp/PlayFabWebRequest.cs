...
namespace PlayFab.Internal {
  public class PlayFabWebRequest : ITransportPlugin {
    ...
    private void SimpleHttpsWorker(string httpMethod, string fullUrl, byte[] payload, Action<byte[]> successCallback,
      Action<string> errorCallback) {
    ...
    public void MakeApiCall(object reqContainerObj) {
      var reqContainer = (CallRequestContainer) reqContainerObj;
    ...
    private static void ActivateThreadWorker() {
      lock (_ThreadLock) {
        if (_requestQueueThread != null) return;
    ...
    private static void WorkerThreadMainLoop() {
      try {
        ...
        var localActiveRequests = new List<CallRequestContainer>();
        do {
          ...
          lock (_ThreadLock) {
            ...
            if (!active) _requestQueueThread = null;
    ...
    private static void ProcessHttpResponse(CallRequestContainer reqContainer) {
      try {
        ...
        if (httpResponse.StatusCode == HttpStatusCode.OK) reqContainer.JsonResponse = ResponseToString(httpResponse);
