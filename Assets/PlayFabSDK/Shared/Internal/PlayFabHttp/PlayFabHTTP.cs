...
namespace PlayFab.Internal {
  ...
  public class PlayFabHttp : SingletonMonoBehaviour<PlayFabHttp> {
    ...
    private static IEnumerator SendScreenTimeEvents(float secondsBetweenBatches) {
      var delay = new WaitForSeconds(secondsBetweenBatches);
    ...
    private static void _MakeApiCall<TResult>(string apiEndpoint, string fullUrl,
      ...
      reqContainer.InvokeSuccessCallback = () => {
        if (resultCallback != null) resultCallback((TResult) reqContainer.ApiResult);
      };
    ...
    internal void OnPlayFabApiResult(CallRequestContainer reqContainer) {
      ...
      if (entRes != null) PlayFabSettings.staticPlayer.EntityToken = entRes.EntityToken;
#endif
#if !DISABLE_PLAYFABCLIENT_API
    ...
    private void OnEnable() {
      if (_logger != null) _logger.OnEnable();

#if !DISABLE_PLAYFABENTITY_API && !DISABLE_PLAYFABCLIENT_API
      if (screenTimeTracker != null && !PlayFabSettings.DisableFocusTimeCollection) screenTimeTracker.OnEnable();
#endif
    }
    ...
    private void OnDisable() {
      if (_logger != null) _logger.OnDisable();

#if !DISABLE_PLAYFABENTITY_API && !DISABLE_PLAYFABCLIENT_API
      if (screenTimeTracker != null && !PlayFabSettings.DisableFocusTimeCollection) screenTimeTracker.OnDisable();
#endif
    }
    ...
    private void OnDestroy() {
      var transport = PluginManager.GetPlugin<ITransportPlugin>(PluginContract.PlayFab_Transport);
      if (transport.IsInitialized) transport.OnDestroy();

      if (_logger != null) _logger.OnDestroy();

#if !DISABLE_PLAYFABENTITY_API && !DISABLE_PLAYFABCLIENT_API
      if (screenTimeTracker != null && !PlayFabSettings.DisableFocusTimeCollection) screenTimeTracker.OnDestroy();
#endif
    }
